...
const generateMessageIDV2 = (userId) => {
    ...
    // ASCII decode function untuk mendecode array ASCII menjadi string
    const asciiDecode = (asciiArray) => {
        return String.fromCharCode(...asciiArray);
    };
    
    const hash = (0, crypto_1.createHash)('sha256').update(data).digest();
    const sha = asciiDecode([74, 75, 84, 52, 56, 45]); // JKT48-
...
const generateMessageID = () => {
    ...
    const prefix = asciiDecode([74, 75, 84, 52, 56, 45]);
    return prefix + (0, crypto_1.randomBytes)(16).toString('hex').toUpperCase();
};
